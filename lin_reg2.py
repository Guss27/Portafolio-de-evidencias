...
def suma(x):            	
# Función "suma" con x como parámetro
...
(Length)
 ...
 sum = x[i] + sum	
# incrementa el valor de "sum"
    return sum

print ("SUMATORIA DE X:", suma(x))	
# Aplica la función "suma" para los diferentes valores de x (lista)
print ("SUMATORIA DE Y:", suma(y))	
# Aplica la función "suma" para los diferentes valores de y (lista)

import pandas as pd			
# Importa pandas como pd

def lin_reg(m,n):		
	# Función "lin_reg" con m y n como parámetros (x,y)

    x = pd.array(m)		
	# Muestra los valores de x (convertidos en array para poder realizar las operaciones directamente entre los arreglos).
    print("VALORES DE X:", x)

    y = pd.array(n)		
	# Muestra todos los valores (elementos) del arreglo de y
    print("VALORES DE Y:", y)

    n = len(m)				
# Muestra y guarda la longitud del arreglo 
(necesario para su aplicación en la fórmula)
    print("LONGITUD DEL ARREGLO:",n)

    xy=x*y				
# Multiplica los arreglos "x" y "y".
    print("VALORES DE X*Y:", xy)

    x2 = x*x				
# Multiplica el arreglo x por sí mismo
    print("VALORES DE X^2:", x2)

    sumx2 = (suma(x)*suma(x))	
	# Realiza la sumatoria de los valores de x y posteriormente, la eleva al cuadrado
 ...
 #numerador_a y denominador_a aplicados en la fórmula a_nd
